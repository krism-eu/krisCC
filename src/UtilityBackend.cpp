...
{
    return Validators::flatpakId(name.trimmed());
}

bool UtilityBackend::validContainerName(const QString &name) const
{
    return Validators::containerName(name.trimmed());
}
...
bool UtilityBackend::runPodman(const QString &mode, const QString &container, const QString &value)
{
    ...
    if (mode == QStringLiteral("remove"))
        return start(QStringLiteral("/usr/bin/podman"), {QStringLiteral("rm"), name},
                     tr("Elimina container: %1").arg(name), QStringLiteral("podman.remove"), kPodmanActionTimeoutMs);
